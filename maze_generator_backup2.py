    ...
    def generate(self, screen=None, animate=False):
        ...
        while unvisited:
            try:
                ...
                if stack:
                    cur = stack.pop()
        if isinstance(self.grid[1][1], Wall): self.grid[1][1] = Cell(1,1,self)
    ...
    while agent != (goal.x,goal.y):
        ...
        for i in range(len(path)): # assumes that path[0] is the start and thus cannot be a wall
            ...
            pygame.draw.rect(screen, (0, 255, 0),
                         (actual_maze.get(agent[0],agent[1]).rect.x, actual_maze.get(agent[0],agent[1]).rect.y, Cell.w, Cell.h))
