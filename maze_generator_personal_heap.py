...
def maze_reset(screen, maze):
    for x in range(maze.w):
        for y in range(maze.h):
            if isinstance(maze.grid[x][y], Cell) and screen.get_at((maze.grid[x][y].rect.x, maze.grid[x][y].rect.y)) == (0, 0, 180) or screen.get_at((maze.grid[x][y].rect.x, maze.grid[x][y].rect.y)) == (51, 153, 255) or screen.get_at((maze.grid[x][y].rect.x, maze.grid[x][y].rect.y)) == (255, 0, 0) or screen.get_at((maze.grid[x][y].rect.x, maze.grid[x][y].rect.y)) == (0, 255, 0):
...
def find_random_cell(maze) -> Cell:
    while True:
        x = random.randint(1,maze.w-2)
        y = random.randint(1, maze.h-2)
        if not isinstance(maze.grid[x][y], Wall):
...
def big_g_vs_little_g(screen, num_mazes):
    ...
    for i in range(int(num_mazes)):
        ...
        start = find_random_cell(maze) 
        goal = find_random_cell(maze)
        if i % 25 == 1: maze = make_maze_unsolvable(screen, maze, goal)
