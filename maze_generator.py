...
def generate_vision_maze(agent: Cell, actual_maze: Maze) -> Maze:
    ...
    vision_maze.grid = [[Wall(x, y, vision_maze) for y in range(
        vision_maze.h)] for x in range(vision_maze.w)]  # initializes all cells to Wall
    for x in range(1, vision_maze.w-1):
        for y in range(1, vision_maze.h-1):
            # makes all cells except boundary cells Fog
            vision_maze.grid[x][y] = Fog(x, y, vision_maze)
...
def find_priority(start: Cell, goal: Cell, g: dict, big_g_pref: bool) -> int:  # not sure if works 100%
    ...
    # constant bigger than any possible generated g value
    c = start.maze.w * start.maze.h
    # packing a random large number to break remaining ties
    random_tie_breaker = random.randint(1, 10**20-1)
    if big_g_pref:
        # f value, g tiebreaker, random value tiebreaker
...
def repeated_forward_a_star_search(screen, actual_maze: Maze, vision_maze: Maze, start: Cell, goal: Cell):
    pygame.draw.rect(screen, (255, 0, 0), (actual_maze.get(
        goal.x, goal.y).rect.x, actual_maze.get(goal.x, goal.y).rect.y, Cell.w, Cell.h))
    ...
    # key: cell coords tup, value: that cell's parent cell coords tup (useful for retracing paths)
    tree = {}
...
def repeated_forward_a_star_search_experiment(screen, actual_maze: Maze, vision_maze: Maze, start: Cell, goal: Cell, big_g_pref: bool) -> int:
    ...
    while agent != (goal.x, goal.y):
        ...
        closed_list = []  # a list of already visited cell coordinate tuples
        # put start into open list with f value as priority
        heapq.heappush(open_list, (find_priority(
            start, goal, g, big_g_pref), agent))
        num_expanded_cells += compute_path(
            vision_maze, goal, open_list, closed_list, g, tree, search, counter, big_g_pref)
        if open_list == []:
            print("I cannot reach the target")
            return
        """
...
def maze_list_experiment_backup(screen, maze_list):
    ...
    for maze in maze_list:
        ...
        goal = maze.get(maze.w - 2, maze.h - 2)
        vision_maze = generate_vision_maze(start, maze)
        expanded_cells_list_big_g.append(repeated_forward_a_star_search_big_g(
            screen, maze, vision_maze, start, goal))


def maze_reset(screen, maze):
    for x in range(maze.w):
        for y in range(maze.h):
            if isinstance(maze.grid[x][y], Cell) and screen.get_at((maze.grid[x][y].rect.x, maze.grid[x][y].rect.y)) == (0, 0, 180):
                pygame.draw.rect(
                    screen, (255, 255, 255), (maze.grid[x][y].rect.x, maze.grid[x][y].rect.y, Cell.w, Cell.h))
                pygame.display.update()
                pygame.event.pump()
...
def maze_list_experiment(screen, num_mazes):
    ...
    for i in range(int(num_mazes)):
        ...
        vision_maze = generate_vision_maze(start, maze)
        expanded_cells_list_big_g.append(repeated_forward_a_star_search_experiment(
            screen, maze, vision_maze, start, goal, big_g_pref=True))

        maze_reset(screen, maze)

        vision_maze = generate_vision_maze(start, maze)
        expanded_cells_list_little_g.append(repeated_forward_a_star_search_experiment(
            screen, maze, vision_maze, start, goal, big_g_pref=False))
    ...
    big_g_average = sum(expanded_cells_list_big_g) / \
        len(expanded_cells_list_big_g)
    print("average number of expanded cells for repeated forward A* on", num_mazes,
          maze.w-2, "x", maze.h-2, "graphs with larger g-value tiebreaking:", big_g_average)

    print("sum of expanded cells list little g:",
          sum(expanded_cells_list_little_g))
    print("len of expanded cells list little g:",
          len(expanded_cells_list_little_g))
    little_g_average = sum(expanded_cells_list_little_g) / \
        len(expanded_cells_list_little_g)
    print("average number of expanded cells for repeated forward A* on", num_mazes,
          maze.w-2, "x", maze.h-2, "graphs with smaller g-value tiebreaking:", little_g_average)
...
def forward_vs_backward(screen, num_mazes):
    ...
    for i in range(int(num_mazes)):
        ...
        goal = maze.get(maze.w - 2, maze.h - 2)
        vision_maze = generate_vision_maze(start, maze)
        expanded_cells_list_forward.append(repeated_forward_a_star_search_experiment(
            screen, maze, vision_maze, start, goal, big_g_pref=True))
        ...
        goal = maze.get(1, 1)
        vision_maze = generate_vision_maze(start, maze)
        expanded_cells_list_backward.append(repeated_backward_a_star_search_experiment(
            screen, maze, vision_maze, start, goal, big_g_pref=True))

    print("sum of expanded cells list forward repeated A*:",
          sum(expanded_cells_list_forward))
    print("len of expanded cells list forward repeated A*:",
          len(expanded_cells_list_forward))
    forward_average = sum(expanded_cells_list_forward) / \
        len(expanded_cells_list_forward)
    print("average number of expanded cells for repeated forward A* on", num_mazes,
          maze.w-2, "x", maze.h-2, "graphs with larger g-value tiebreaking:", forward_average)

    print("sum of expanded cells list backward repeated A*:",
          sum(expanded_cells_list_backward))
    print("len of expanded cells list backward repeated A*:",
          len(expanded_cells_list_backward))
    backward_average = sum(expanded_cells_list_backward) / \
        len(expanded_cells_list_backward)
    print("average number of expanded cells for repeated backward A* on", num_mazes,
          maze.w-2, "x", maze.h-2, "graphs with larger g-value tiebreaking:", backward_average)
